...
pickPred()
{
    const char* pred = PREDS[pick(0, NUM_PREDS - 1)];
    if (strcmp(pred, "=") == 0) return pickZeroOne() ? "=" : "==";
    return pred;
}

static int
pickK(int len, const char* pred)
{
    if (strcmp(pred, "<") == 0)
    {
        return pick(1, len + 1);
...
"in comments before the 'p cnf ...' header.\n"
	...
	options = argv[i];
        ...
        if (seed < 0)
        {
            SYSTEMTIME st;
            GetSystemTime(&st);
            seed = abs((int)st.wSecond * (int)GetCurrentProcessId() >> 1);
        }
    #else
